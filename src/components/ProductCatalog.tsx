      ...
      {showCart && (
        ...
        >
                ...
                return (
                  <div key={category}>
                    ...
                    <div className="flex items-center justify-end mt-2 px-1">
                      <span className="text-xs text-gray-500">小计：</span>
                      <span className="text-xs font-semibold text-orange-600 ml-1">${categoryTotal.toFixed(2)}</span>
                    </div>
                  </div>
                );
              });
          ...
          <div className="px-4 py-3 border-t border-gray-100 bg-gray-50/80">
            {(() => {
              // 检测是否有多个类型
              const categoryCount = new Set(cartItems.map(i => i.categoryName || '其他')).size;
              return (
                <>
                  {categoryCount > 1 && (
                    <div className="flex items-center gap-2 mb-2 px-2 py-1.5 bg-orange-50 rounded-lg">
                      <svg className="w-4 h-4 text-orange-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      <span className="text-xs text-orange-700">将拆分为 {categoryCount} 个报价单</span>
                    </div>
                  )}
                  <div className="flex items-center justify-between mb-2">
